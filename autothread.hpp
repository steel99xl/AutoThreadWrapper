...
struct Thread{
	...
	bool AutoLoop = false; //If set to true your function thread will run on a loop untill told to stop
		...
		do {	
			switch(T->State){
				...
				case WAIT:{	
							  ...
							  // This sets the local tmp  value without effecting the wait time if wanted for auto loop
							  tmp = 0;
